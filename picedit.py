# -*- coding: utf-8 -*-
"""
Created on Mon Nov  9 12:43:05 2020
...
"""
clampInt
    Clamps the input integer value to be
    between the input bounds
    
Parameters:
    _value  : The value to clamp
    _min    : The smallest integer that _value can take
    _max    : The largest integer that _value can take

Returns:
    _value such that _min <= _value <= _max, _value is an integer
"""
...
def atOrAboveLowerBound(_value, _lowerBound):
...
"""
inBounds
    Checks if the input x and y coordinates 
    are within the input bounds.
    
Parameters:
    _x      : the x coordinate to check
    _y      : the y coordinate to check
    _maxX   : one more than the largest value _x can take
    _maxY   : one more than the largest value _y can take
    _minX   : smallest value _x can take (Defaults to 0)
    _minY   : smallest value _y can take (Defaults to 0)

Returns:
    True if x and y are within the bounds given
    False if either x or y are outside the bounds given
"""
def inBounds(_x, _y, _maxX, _maxY, _minX = 0, _minY = 0):
    return atOrAboveLowerBound(_x, _minX) and atOrAboveLowerBound(_y, _minY) and belowUpperBound(_x, _maxX) and belowUpperBound(_y, _maxY)
...
"""
getPixelNeighbours
    Gets the neighbours of the pixel at (_x, _y)
    This function does not consider pixels that are not 
    surrounded by 8 other pixels

Parameters:
    _image : The image that the program is working on
    _x     : The x coordinate of the "root" pixel
    _y     : The y coordinate of the "root" pixel

Returns:
    The 2D (3x3) matrix of neighbours. 
    Note that the (1, 1) element is the root of the
    matrix
"""
def getPixelNeighbours(_image, _x, _y):
    return [[_image[_x + x, _y + y] for x in range(-1, 2)] for y in range(-1, 2) if inBounds(_x, _y, _image.shape[0], _image.shape[1], 0, 0)]

"""
getAllNeighbours
    Gets the neighbours of all pixels 
    This function does not consider pixels that are not 
    surrounded by 8 other pixels
...
"""
getConvolutionSum
    Iterates through both the neighbour 2D array and
    the kernel to calculate the convolution sum

Parameters:
    _neighbours : The 2D (3x3) matrix of neighbours. 
                  Note that the (1, 1) element is the root of the
                  matrix
    _kernel     : The 2D (3x3) matrix of values
    _colour     : The current colour channel to calculate on 

Returns:
    The sum of the convolution between the neighbours matrix and 
    the kernel
"""
...
applyConvolutionToImage
    Iterates through the image, calculates the convolution sum 
    of each colour channel of each pixel, and then saves that
    value into a new image of the same dimensions

Parameters:
    _image         : The image that the program is working on
    _allNeighbours : The 2D array of neighbours
    _kernel        : The 2D (3x3) matrix of values

Returns:
    A new image (3D array) made using the convolution sum
"""
def applyConvolutionToImage(_image, _allNeighbours, _kernel):
    ...
    for x in range(0, _image.shape[0]):
        for y in range(0, _image.shape[1]):
            if inBounds(x, y, _image.shape[0], _image.shape[1], 0, 0):
...
"""
convolutionFilter
    Applies the convolution function onto an image and then
    optionally brightens it up

Parameters:
    _image    : The image that the program is working on
    _kernel   : The 2D (3x3) matrix of values
    _brighten : Whether to brighten the image

Returns:
    A new image (3D array) that is optionally brightened
"""
    ...
    if _brighten:
        for x in range(0, len(newImage)):
            for y in range(0, len(newImage[x])):
                if inBounds(x, y, len(newImage), len(newImage[x]), 0, 0):
...
def getDirectNeighbours(_x, _y, _maxX, _maxY, _minX = 0, _minY = 0):
    ...
    if atOrAboveLowerBound(_x, _minX):
        neighbours.append((_x - 1, _y))
    if atOrAboveLowerBound(_y, _minY):
    ...
    while run:
        if (imageLoaded):
            ...
            if option == 'e': 
                    ...
                    while True:
                        ...
                        for char in newName:
                            if char in delimiters:
                                print("You have entered an inavlid file name.")
                                hasDelimeter = True
                                break
                        if hasDelimeter == False:
                ...
                changedImage = change_brightness(newImg, getBoundedInt(-255, 256))
                ...
                changedImage = change_contrast(newImg, getBoundedInt(-255, 256))
